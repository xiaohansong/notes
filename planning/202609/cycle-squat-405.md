# Cycle — back squat 405# (sole focus)

**TL;DR**

- Goal: **405# back squat**, attempt window early–mid Dec. Best banked: 390 × 1 (5/19, RPE 9.5)
- Working **e1RM = 395#** — read off the *first* set at 325 (RPE 8–8.5). The 9.5 was set 3, fatigue-limited, and doesn't estimate a max
- Squat only. No press/DL/bench ladders this cycle; CrossFit is noise, never advances the ladder
- 2 squat days/wk, ~72h apart: **Volume slot** (5s, 75–80%) + **Intensity slot** (5s→3s→2s, 85–95%)
- 4 phases: consolidate (wk 5–6) → build 5s/3s (wk 7–12) → doubles (wk 13–15) → attempt
- Hard gate: **1–3 wk of doubles @ 92–95% before any attempt.** No 3s → 1RM jump, ever

## Where the block actually is

Weeks 1–4 (8/10 → 9/3) were re-accumulation after the July layoff, and they worked:

| wk | volume slot | intensity slot | tonnage |
|---|---|---|---|
| 1 | 255 × 10/9/7/6 | 235 × 5×8 | 17,560 |
| 2 | 255 × 10/10/8/8 | 255 × 5×6 | 16,830 |
| 3 | 265 × 5×8 | 295 × 4×6 | 17,680 |
| 4 | 305 × 6×4 | **325 × 5×3** | 12,195 |

Tonnage held ~17k for three weeks while load climbed 235 → 295, then wk 4 cut volume 31% and
took load to 325. That is a textbook volume→intensity conversion, whether or not it was planned.

**But +90# in four weeks was reclaiming known ground, not building new ground** — and the RPE
data confirms exactly where it ran out. RPE sat at 7 through wk 4 day 1, then hit 9.5 on wk 4
day 2. The ceiling arrived in a single session. From here the ladder moves in fives.

## The e1RM, now measured (RPE added 9/3)

| session | load × reps | RPE | implied 1RM |
|---|---|---|---|
| wk 3 d2 | 295 × 6 | 7 | 387# |
| wk 4 d1 | 305 × 4 | 7 | 378# |
| wk 4 d2 | 325 × 5, **set 1–2** | **8–8.5** | **394–401#** |
| wk 4 d2 | 325 × 5, set 3 | 9.5 | 382# — fatigued, discard |

**Working e1RM = 395#.** Estimate off the *fresh* set: set 3 of a session measures accumulated
fatigue, not maximal strength, so the 9.5 is a within-session drop-off rather than a max reading.
The RPE 7 entries pull lower (387, 378), but RPE is least reliable that far from failure —
three-reps-in-reserve is a guess, half-a-rep is a fact. Weight the 8–8.5.

What the *shape* says matters as much as the number: **top-end load is fine; fatigue resistance is
the gap.** A set-1 RPE 8 that degrades to a grinding 9.5 by set 3 is a work-capacity signal, not a
strength ceiling. That argues for accumulating quality volume at 80–87% — which is exactly what
this cycle is built on, and it's the cheapest +10# of e1RM available before December.

Same-bar, different-target still holds: 325 is an RPE 7 triple and an RPE 8.5 set of five.

## Checking the rest of the program

Weeks 5 and 6 of the existing program, checked against 395:

| session | load × reps × sets | vs 395 e1RM | verdict |
|---|---|---|---|
| wk 5 d1 | 305 × 5 × 5 | 77.2% → RPE 6.5 | run it — light, but it follows a hard day |
| wk 5 d2 | 325 × 3 × 3 | 82.3% → RPE 7 | run it — deliberate re-entry |
| wk 6 d1 | 330 × 5 × 5 | 83.5% → RPE 9 | runnable, but 25 reps at RPE 9 |
| wk 6 d2 | 345 × 3 × 5 | 87.3% → RPE 9 | runnable, but 15 reps at RPE 9 |

Week 5 is well-placed. **Week 6 is not impossible — it is a whole week at RPE 9, on both days.**
The loads are close to right; the set counts are what make it punishing. Trim volume, keep the bar
weight roughly where the program wants it.

### "But the program expects me to grow"

Correct, and it should — a pre-written block prescribes against a *projected* max. With the e1RM
read properly at 395, the program's projection is close to right and the earlier "not available"
call was wrong.

What's left is a volume problem, not a load problem. Week 6 as written runs **both** days at
RPE 9, for 25 and 15 reps. One RPE-9 top set is training; a whole week of RPE-9 sets is how
week 4 day 2 happened — and that session moved the e1RM by nothing while costing a fatigue tail.

Fixed by trimming sets, keeping the loads near the program's intent:
- wk 6 volume → **320 × 5 × 5** (RPE 8, −10# on the program's 330)
- wk 6 intensity → **345 × 3 × 4** (RPE 9 top end, one set cut from five)

## The ladder, with growth built in

Projected at ~+1.5#/wk on the e1RM. Intensity slot = triples at 85–88%, volume slot = fives at
~81%. Recomputed at each checkpoint against actual logged RPE — projections, not promises.

| wk | proj. e1RM | intensity slot | volume slot |
|---|---|---|---|
| 5 | 395 | 325 × 3 × 3 | 305 × 5 × 5 |
| 6 | 397 | 345 × 3 × 4 | 320 × 5 × 5 |
| 7 | 398 | 340 × 3 × 5 | 320 × 5 × 5 |
| 8 | 400 | 345 × 3 × 5 | 325 × 5 × 5 |
| 9 | 401 | 350 × 3 × 4 | 325 × 5 × 5 |
| 10 | 403 | 350 × 3 × 5 | 330 × 5 × 5 |
| 11 | 404 | 355 × 3 × 4 | 330 × 5 × 5 |
| 12 | 406 | 355 × 3 × 5 | 335 × 5 × 4 |
| 13 | 408 | **doubles 378 × 2** | 330 × 5 × 3 |
| 14 | 409 | **doubles 385 × 2** | 320 × 5 × 3 |
| 15 | 411 | **doubles 390 × 2** | light |
| 16 | ~412 | **attempt 375 / 395 / 405** | — |

**405 is comfortably live.** From 395 it needs only +10# of e1RM over 11 weeks — under +1#/wk
clears it. At +1.5#/wk the projection lands near 412, which puts 405 inside the taper rather than
at the edge of it, and leaves a fourth attempt on the table if the day goes well.

Note what the ladder does *not* do: outside the doubles phase it keeps sets at RPE 8–9, never
9.5+. Week 4 day 2 is the cautionary case — the grinding third set bought no strength and cost a
week of freshness. Stop the set when the bar slows, not when it fails.

## Phases

1. **Re-enter at honest loads (wk 5–6).** Intensity slot moves to triples at 85%; volume slot
   climbs in fives. Ladder steps are +5#, not +25#.
2. **Build (wk 7–12).** Intensity slot walks 325 → ~345 in triples as the e1RM rises. Volume slot
   walks 310 → ~330 in fives. All percentages recomputed off the current e1RM, never off 405.
3. **Doubles (wk 13–15).** 92–95% of the revised TM. This is the CNS-priming phase and it is
   non-negotiable ([[feedback_1rm_test_protocol]]).
4. **Attempt (wk 16).** Openers ~365 / 385 / **405**. Target RPE 9–9.5 — a max attempt is not an
   RPE 8 lift ([[feedback_rpe_math]]).

## What a block like this realistically returns

Gains scale inversely with training age, and at **2.17× bodyweight (395 / 182#)** you sit on the
intermediate/advanced boundary. That band is well past linear progression.

| training age | typical squat gain |
|---|---|
| novice (0–1 yr) | 10–20#/**month**, linear |
| intermediate (1–3 yr) | 5–10#/month |
| **advanced (>2× BW)** | **10–25#/year** |

Your own history sits right on that: the spring block ran 375 → **390 tested** over ~5 weeks
(caveat — the 375 was a recorded number, not a fresh test the same week, so the real gain was
probably smaller). Then 390 → 395 across 3.5 months that contained a 7.5-week layoff.

**Realistic for the remaining 11 weeks: +5 to +15#.** That puts the base case at **400–408**, with
405 a genuine stretch rather than a formality, and 415 a very good block.

The reason it isn't more: **the bodyweight lever is already spent.** BW is back to 182 (the prior
peak) as of 9/3, so the 395 e1RM already contains it. Everything from here is training-driven
strength *above a lifetime best* — the expensive kind.

What that does *not* mean is +10#/quarter forever. +10 in 11 weeks annualizes to ~12%/yr, which no
advanced lifter sustains. Expect 2027 to move in single-digit annual increments.

### The levers, ranked by cost

1. **Uninterrupted weeks.** 11 clean weeks would be the longest unbroken squat block in the log.
   The 7.5-week summer layoff cost more than any programming choice will win back.
2. **Fatigue resistance.** Identified 9/3: set 1 at RPE 8 → set 3 grinding at 9.5. Trainable on a
   scale of weeks, not months, and it's what the 80–87% volume work is for. Best available lever
   now that bodyweight is spent.
3. **The left hip.** A structural limiter with the widest error bars of anything here — resolving
   it could be worth more than everything above, or nothing. The hip block is the cheap bet on it.
4. **More bodyweight — available, but now a decision.** 182 → 187 is worth roughly **+7#** of
   squat, 182 → 190 about +11#. That is no longer a free glide back to a prior peak; it's a
   deliberate gaining phase, and it belongs to you to choose, not to the program to assume.
   Food is training substrate either way — but note that 405 at 182 and 405 at 190 are different
   lifts to buy.
5. **Programming.** Real, but last. Good programming mostly protects the other four from being
   squandered; it doesn't manufacture strength on its own.

### The pre-program weeks matter

Three unlogged weeks before wk 1 hovered at 275 / 295 / 305 for 5s with no progression. Against
the 395 e1RM those are **69–77%, i.e. RPE 5–7** — you were sitting well under the loads needed to
drive adaptation, which is why nothing moved. That's a stall from under-loading, not a ceiling.

Two things follow. **The coached program was the right call and it worked** — it took a stalled
lifter and moved 305 → 325 × 5 in four weeks. The wk-6 criticism in this doc is about set counts
at RPE 9, not about the program's direction. And the real block is ~7 weeks old, not 4, which is
worth remembering when fatigue accumulates in October.

## Standing rules

- **Percentages come off the measured e1RM, never off the goal.** This is the single error that
  broke wk 6, and it will re-break wk 9 and wk 12 if the habit survives.
- **Hesitation = bail.** A cut set at 325 is a correct read, not a missed PR. Bailing costs one
  session; a hip that folds costs the December attempt.
- **Left hip is the gate.** Forefoot drift under load is the known failure mode and it is what
  ended 5/16 at 335 × 2. Any drift on a top set ends the set — not the rep, the set.
- **Log RPE per set.** Every gate in this cycle reads it. A session logged without RPE can't
  advance the ladder, so it wastes the week.
- One variable at a time. If a session struggled, don't hold load *and* add anything new.
- 90 min cap including warm-up.
